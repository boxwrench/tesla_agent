...
|---|---|---|---|---|---|---|
| **gpt-oss-120B MXFP4** — AMERICAN-ONLY quality/speed (US-origin, OpenAI) | **Vulkan RADV** | **Pairwise 5-1 vs Qwen 35B; 4-2 vs Qwen 122B** | **~46 tok/s** | not captured in stable run | ~63 GB | **3 / 3 Pass** |
| **Gemma 4 31B IT Q6_K** — AMERICAN-ONLY coding second-opinion (US-origin, Google; dense — slow decode) | **Vulkan RADV** | **Pairwise 4-2 vs Gemma 26B-A4B** | **~8.25 tok/s tg128; ~7.7 tok/s sustained long completions** | pp8192 ~133.6 tok/s (verified) | 25.2 GB | **3 / 3 Pass** |
| **Qwen 3.6 35B MoE (Think-On)** — **CODE/general workhorse (default)** | **Vulkan RADV** | **82 / 84** | **~58.5 tok/s** | **932.1 tok/s** | 21.7 GB | **3 / 3 Pass** |
...
| **StepFun Step-3.7-Flash + Q8_0 MTP draft** — QUALITY champion (graduated 2026-06-02) | **Vulkan RADV, patched llama.cpp b9360** | **Plain StepFun pairwise: 6-0 vs gpt-oss-soulfix; 4-0-2 vs 122B** | **27.9 tok/s** (wall std 78.0 s) | **183.5 tok/s** | 88.79 GiB + 3.5 GB draft | **3 / 3 Pass; coding PASS 5/5 E2E** |
| **StepFun Step-3.7-Flash plain** — QUALITY champion, plain (no-draft) lane | Vulkan RADV, llama.cpp b9360 | **6-0 vs gpt-oss-soulfix; 4-0-2 vs 122B** | **20.4-22.3 tok/s** | **212.0 tok/s** | 88.79 GiB | **3 / 3 Pass; coding 4/5 E2E** |
| **Qwen 3.5 122B MoE (Think-On)** — *retired 2026-06-02* | ROCm 7.2.x | **80 / 84** | **19.4 tok/s** | ~136.0 tok/s | 70.0 GB | **3 / 3 Pass** |
| **Qwen 3.5 122B MoE (Think-Off)** — *retired 2026-06-02* | ROCm 7.2.x | **81 / 84** | **19.5 tok/s** | ~136.0 tok/s | 70.0 GB | **3 / 3 Pass** |
| **Qwen 3.5 122B MoE MTP (DRAFT_N=1, PMIN unset)** — *retired 2026-06-02 (tuned Vulkan lane; kept as record)* | **Vulkan RADV, llama.cpp b9360** | **80-81 / 84 role preserved; 3-3 quality tie vs previous MTP config** | **28.3 tok/s** | **324.9 tok/s** | ~70 GB | **3 / 3 Pass; coding PASS 5/5 E2E** |
...
| **Qwen 3.5 35B MoE (Think-On)** — PLAN/AGENTIC baseline | ROCm 7.2.x | **79 / 84** | **47.3 tok/s** | ~562.9 tok/s | 21.0 GB | **3 / 3 Pass** |
...
> **Stable Stack update (2026-06-02 stable set):** **StepFun Step-3.7-Flash graduated as the QUALITY champion** for formal synthesis — MTP lane at **27.9 tok/s** (wall std 78.0 s, ub=256; ubatch sweep 2026-06-06 — +7% tg vs prior ub=512 default), nonce 3/3 and coding 5/5 E2E, plain-pairwise 6-0 vs gpt-oss-soulfix and 4-0-2 vs 122B. It **replaced Qwen 3.5 122B**, which is now **retired** (kept on disk for regression only; the tuned native-MTP Vulkan lane had lifted it from ~19.4 to 28.3 tok/s with `DRAFT_N=1` / `PMIN` unset). **gpt-oss-120B and Gemma 4 31B are now the AMERICAN-ONLY tier** — US-origin models (OpenAI and Google) for agencies that may require domestic-only model provenance; gpt-oss-120B is the quality/speed pick (5-1 vs Qwen 35B, 4-2 vs Qwen 122B) and Gemma 31B Q6 the coding second-opinion (dense — quality verification, not throughput). The CODE/general workhorse is Qwen 3.6 35B and the PLAN/AGENTIC baseline is Qwen 3.5 35B. Gemma 26B-A4B QAT Q4_0 is the fastest general Gemma lane at 59.4 tok/s decode / 1194.4 tok/s prefill, while the older 26B-A4B UD-Q6_K_XL remains the verified non-QAT control at 44.76 ± 0.90 tok/s tg128 / pp512 1002.76 ± 10.29 tok/s. Qwen3-Coder-Next moved from the old ROCm row to the promoted Vulkan b9360 row at 44.4 tok/s decode / 723.2 tok/s prefill.
>
> *Public-mirror note: StepFun's source/checksum pins are not yet complete in this public mirror, so it is measured here but not yet turnkey-reproducible from the public repo alone — see the Model Artifact Manifest.*
>
...
| Prefill speed | StepFun Step-3.7-Flash MTP, Vulkan/RADV | **183.5 tok/s** | ub=256 sweep (2026-06-06); paired baseline measured 212.0 tok/s |
...
| MTP draft acceptance | StepFun Step-3.7-Flash + Q8_0 MTP draft | **89.3%** | ub=256 sweep (2026-06-06); `full_bench.sh --mtp` probe; prior graduation run was 84.7% from raw `tg_probe.json` counters |
