...
|---|---|---|---|---|---|---|
| **StepFun Step-3.7-Flash MTP** | 88.79 GiB + 3.5 GB draft | 12,288 | Plain StepFun pairwise: **6-0 vs gpt-oss-soulfix; 4-0-2 vs 122B** | **27.9 tok/s** (Vulkan; pp 183.5 tok/s; wall std 78.0 s; ub=256) | **3 / 3 Pass** | **QUALITY champion (graduated 2026-06-02)**; replaced Qwen 122B at the same decode speed with higher quality. *(Public source/checksum pins still pending.)* |
| **Qwen 3.6 35B MoE (Vulkan RADV)** | 21.7 GB | 32,768 | **82 / 84** | **~58.5 tok/s** (RADV) | **3 / 3 Pass** | **CODE/general workhorse (default)** |
| **Qwen 3.5 35B-A3B MoE (MXFP4)** | 21.0 GB | 32,768 | **79 / 84** | **47.3 tok/s** (ROCm) | **3 / 3 Pass** | **PLAN/AGENTIC baseline** |
| **gpt-oss-120B MXFP4 (3 shards)** | ~63 GB | 32,768 | **5-1 vs Qwen 35B; 4-2 vs Qwen 122B** | **~46 tok/s** (RADV) | **3 / 3 Pass** | **AMERICAN-ONLY quality/speed** — US-origin (OpenAI) lane for agencies that may require domestic-only model provenance; needs the draft-with-assumptions system prompt |
| **Gemma 4 31B IT Q6_K** | 25.2 GB | 32,768 | **4-2 vs Gemma 26B-A4B** | **~8.25 tok/s tg128; ~7.7 tok/s sustained** (Vulkan; pp8192 ~133.6 tok/s) | **3 / 3 Pass** | **AMERICAN-ONLY coding second-opinion** — US-origin (Google), dense/slow; orchestrated coding path required |
...
| **Qwen 3.5 122B MoE (MXFP4)** | 70.0 GB | 12,288 | **80 / 84** | **19.4 tok/s** (ROCm) | **3 / 3 Pass** | **Retired 2026-06-02** — superseded by the StepFun champion; regression only |
| **Qwen 3.5 122B MoE (MXFP4)** *think-off* | 70.0 GB | 12,288 | **81 / 84** | **19.5 tok/s** | **3 / 3 Pass** | **Retired 2026-06-02** |
| **Qwen 3.5 122B MoE MTP (MXFP4_MOE)** | ~70 GB | 12,288 | **3-3 quality tie vs previous MTP config** | **28.3 tok/s** (Vulkan; pp 324.9 tok/s) | **3 / 3 Pass** | **Retired 2026-06-02** (tuned lane; `DRAFT_N=1`, `PMIN` unset; kept as record) |
| **StepFun Step-3.7-Flash plain** | 88.79 GiB | 16,384 gate / 32,768 coding | **6-0 vs gpt-oss-soulfix; 4-0-2 vs 122B** | **20.4-22.3 tok/s** (Vulkan; pp 212.0 tok/s) | **3 / 3 Pass** | QUALITY champion — plain (no-draft) lane; coding 4/5 E2E |
...
| **Qwen3-Coder-Next (UD-Q4_K_XL)** | 49.6 GB | 32,768 | orchestrated coding artifact: saved grader checks PASS | **44.4 tok/s** (Vulkan; pp 723.2 tok/s) | 3 / 3 Pass recorded | HARD-CODE challenger (128GB-class); Vulkan b9360 promoted |

**Recommendation note:** Qwen remains a strong and widely favored reasoning family, but this guide's default ladder follows the local Strix Halo agent gates. As of the 2026-06-02 stable set: **StepFun Step-3.7-Flash is the QUALITY champion** (it replaced Qwen 122B, now retired, at the same decode speed with higher quality); **Qwen 3.6 35B is the CODE/general workhorse** and **Qwen 3.5 35B the PLAN/AGENTIC baseline**; and **gpt-oss-120B (OpenAI) + Gemma 4 31B (Google) form the AMERICAN-ONLY tier** — US-origin models for agencies that may require domestic-only model provenance. Gemma 4 26B-A4B QAT Q4_0 is the fast Gemma lane at 59.4 tok/s decode / 1194.4 tok/s prefill; the QAT MTP rows are speed probes until QAT-matched assistant heads and quality controls are available.
...
* **CODE / general workhorse (default):** Qwen 3.6 35B-A3B MXFP4, SHA256 `2fdd20997c4d88ee25f70f500c61f8b999378d92ab055f9d450fc70d617158d3`
* **PLAN / AGENTIC baseline:** Qwen 3.5 35B-A3B MXFP4, SHA256 `0f135a59159030f4710477abc6f9922d2f13552c85bff736deaaef71023cd770`
...
* **QUALITY champion (graduated 2026-06-02):** StepFun Step-3.7-Flash UD-IQ4_XS plus `Step-3.7-Flash-MTP-Q8_0.gguf` draft; replaced Qwen 122B at the same decode speed with higher quality. Public source/checksum pins are still incomplete, so treat it as measured but not yet turnkey-reproducible from this public mirror alone.
* **AMERICAN-ONLY quality/speed** (US-origin lane for agencies that may require domestic-only model provenance): gpt-oss-120B MXFP4 (OpenAI), three shards with per-shard SHA256 pins in [reproducibility-matrix.md](reproducibility-matrix.md)
* **AMERICAN-ONLY coding second-opinion** (US-origin, dense): Gemma 4 31B IT Q6_K (Google), SHA256 `abd0be03a2bc3f3c9d8e018cbb4ff5b553c340c65d49b6b346c48be5a1efde28`
* **Retired 2026-06-02:** Qwen 3.5 122B-A10B (MXFP4 and tuned MTP MXFP4_MOE) — superseded by the StepFun champion; retained for regression only.
