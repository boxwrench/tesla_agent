...
### Insight B: Match the Speedup to the Artifact
We attempted to speed up generation using speculative decoding, where a draft path guesses tokens and the main model verifies them.
* The **older separate-draft MoE paths** were slower than normal generation. *Why:* verification forced the large model to reactivate and route different experts constantly, adding a routing penalty that wiped out the gains.
* **Native MTP is different.** Qwen3.6-35B-A3B-MTP GGUFs carry their own `nextn` head, so recent llama.cpp builds can self-speculate with `--spec-type draft-mtp` and no separate draft model. That opt-in lane lifted the 35B workhorse from ~58.5 tok/s to ~72.7 tok/s on MXFP4-MTP, and ~81.2 tok/s on Q4_K_M-MTP. The technique surfaced via the community [strix-halo-guide](https://github.com/hogeheer499-commits/strix-halo-guide), then we re-gated it against this repo's quality bar.
* On **dense** models, DFlash speculative decoding also works. The dense 27B has no expert router, so DFlash lifts the route to **~31 tok/s (2.82×)**. *We benchmarked the dense 27B thoroughly but did **not** keep it in the stack: in practice it was neither faster nor clearly smarter than the 35B MoE workhorse. It stays on the shelf as a "try something different" option for tough, stuck problems — an arrow in the quiver, not a daily driver.*
...
* **Qwen stayed in the stack, but with narrower roles.** Qwen 3.6 35B is the CODE/general workhorse and Qwen 3.5 35B is the PLAN/AGENTIC baseline. (Qwen 122B was the QUALITY spot-specialist at this point — the next pivot changed that.)

### The 2026-06-02 Stable Set: StepFun Champion + AMERICAN-ONLY Tier
The next stable set reorganized the top of the ladder.

* **StepFun Step-3.7-Flash graduated as the QUALITY champion.** The UD-IQ4_XS main model with a Q8_0 MTP draft cleared nonce 3/3 and coding 5/5 E2E, decoding ~27.9 tok/s. It **replaced Qwen 3.5 122B** at the same decode speed with higher quality, so **122B was retired** (kept on disk for regression only).
* **gpt-oss-120B and Gemma 4 31B became the AMERICAN-ONLY tier.** The champion (StepFun) and the Qwen workhorses are non-US in origin. For agencies that may require US-origin models only, this tier names the domestic-origin picks: gpt-oss-120B (OpenAI) for quality/speed and Gemma 4 31B (Google) as the coding second-opinion. Their measured gates are unchanged; what changed is how they are routed.

This is not a claim that Qwen is "bad." Online consensus still treats Qwen as a strong reasoning family, and the local Qwen routes remain the CODE and PLAN baselines. The change is narrower: on this Strix Halo hardware, with these agentic utility workflows, the local gates promoted StepFun to the general quality champion and grouped gpt-oss-120B and Gemma 4 31B as the US-origin (AMERICAN-ONLY) options. For throughput, Qwen 3.6 35B MoE (~58.5 tok/s Vulkan, or ~72.7-81.2 tok/s with opt-in MTP lanes) remains the speed leader; gpt-oss-120B runs ~46 tok/s and Gemma 31B ~8 tok/s on the orchestrated path where quality matters more than wall-clock time.
